...
Watching PID 18401
Attached 16 tracepoints — capturing HTTP traffic...
...
| 参数 | 说明 |
|-----------------|-------------|
| `--direction <DIR>`   | 全部 | 显示方向：`inbound`、`outbound`、`both`（默认） |
...
# 观察 500 响应对
pugi --pid 18401 --status 500

# 只看触发 500 状态的请求本身（不打印响应体）
...
flowchart TB
    subgraph k[内核 — eBPF]
        direction TB
        A["sys_enter_read / readv / recvfrom / recvmsg<br/>↓<br/>save_args(tid, buf, fd)"] --> B["sys_exit_read / readv / recvfrom / recvmsg<br/>↓<br/>bpf_probe_read(buf)<br/>↓<br/>bpf_perf_event_output()"]
    end
...
pugi 挂接了 8 个系统调用的进入和退出（共 16 个 tracepoint）：

| 方向 | 系统调用 | Tracepoint |
|-----------|-----------|--------------------------------|
| 入站 (IN)  | `read`    | `sys_enter_read` / `sys_exit_read`     |
| 入站 (IN)  | `readv`   | `sys_enter_readv` / `sys_exit_readv`   |
| 入站 (IN)  | `recvfrom`| `sys_enter_recvfrom` / `sys_exit_recvfrom` |
| 入站 (IN)  | `recvmsg` | `sys_enter_recvmsg` / `sys_exit_recvmsg` |
| 出站 (OUT) | `write`   | `sys_enter_write` / `sys_exit_write`   |
| 出站 (OUT) | `writev`  | `sys_enter_writev` / `sys_exit_writev` |
| 出站 (OUT) | `sendto`  | `sys_enter_sendto` / `sys_exit_sendto` |
| 出站 (OUT) | `sendmsg` | `sys_enter_sendmsg` / `sys_exit_sendmsg` |

大多数 JVM HTTP 栈使用 `read`/`write` 操作 socket FD。非阻塞框架（Netty、WebFlux、Go net）可能使用 `recvfrom`/`sendto` 或向量/消息 I/O（`readv`/`writev`、`recvmsg`/`sendmsg`）。pugi 已覆盖上述全部。
...
| `perf buffer overflow: N events lost` | 瞬时流量超过 perf 缓冲区容量 | 短暂溢出会自动恢复（pugi 清空连接缓存后继续监听）；若频繁出现，可调大 `perf_event_mlock_kb`：`sudo sysctl -w kernel.perf_event_mlock_kb=32768` |
